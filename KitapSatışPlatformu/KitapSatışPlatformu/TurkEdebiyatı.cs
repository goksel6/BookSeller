...
{
    public partial class TurkEdebiyatı : Form
    {
        Form1 anasayfa;
        İncele inceleForm;
        public TurkEdebiyatı(Form1 a)
        {
            anasayfa = a;
            InitializeComponent();
        }

        private void TurkEdebiyatı_Load(object sender, EventArgs e)
        {
            ...
            commandFransaKitap.CommandText = "SELECT Book_name as KİTAP, Author_name as YAZAR, Publisher_name as YAYINEVİ ,Price as FİYAT  FROM BOOK, AUTHORS,PUBLISHER WHERE Country='Türkiye' AND Author_id=BAuthor_id AND PBook_id=Book_id";
            ...
            try
            {
                connection.Open();
                adapter.Fill(table);
                dataGridViewTürkiye.DataSource = table;
                connection.Close();
            }
            catch
            {
        ...
        private void dataGridViewTürkiye_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            String SecilmisKitapAdi = dataGridViewTürkiye.Rows[e.RowIndex].Cells[0].Value.ToString();
            String SecilmisKitapYayınevi = dataGridViewTürkiye.Rows[e.RowIndex].Cells[2].Value.ToString();
            String SecilmisKitapYazarAdi = dataGridViewTürkiye.Rows[e.RowIndex].Cells[1].Value.ToString();
            int SecilmisKitapFiyat = Convert.ToInt32(dataGridViewTürkiye.Rows[e.RowIndex].Cells[3].Value);
